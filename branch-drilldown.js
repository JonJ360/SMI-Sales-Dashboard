...
const BranchModel = (() => {
  ...
  function monthlyComparison(rows, asOf, first, branch) {
    first=first||'2024-01-01';
    return trend(scope(rows,{period:'FULL'},asOf,first,branch),{period:'FULL'},asOf,first)
      .map(d=>({...d,year:Number(d.name.slice(0,4)),month:Number(d.name.slice(5,7))}));
  }
  function topItems(rows, detail) {
    if (!detail || !detail.lines) return null;
    const groups=new Map();let header=0,lineTotal=0,missing=0;
    for (const doc of rows.filter(d=>d.kind==='Invoice')) {
      ...
      for(const raw of lines) {
        ...
        const item=String(line.item||''),sales=cents(line.sales);
        if(!groups.has(item))groups.set(item,{item,description:line.description||'',gross_sales:0});
        groups.get(item).gross_sales+=sales;lineTotal+=sales;
      }
    }
    const all=[...groups.values()].map(d=>({...d,gross_sales:d.gross_sales/100})).sort((a,b)=>b.gross_sales-a.gross_sales||a.item.localeCompare(b.item));
    return {items:all.slice(0,10),item_count:all.length,line_sales:lineTotal/100,header_sales:header/100,difference:(header-lineTotal)/100,missing_documents:missing};
  }
  return {decode,bounds,scope,totals,group,trend,filter,monthlyComparison,topItems};
})();
...
if (typeof window !== 'undefined') {
  ...
  function renderDetail() {
    ...
    const monthly=BranchModel.monthlyComparison(documents(),state.data.as_of,state.data.invoice_drilldown.start,ui.branch);
    chart('branchMonthlyChart',salespersonMonthlyChart(monthly));
    chart('branchTrendChart',{type:'line',data:{labels:trend.map(d=>d.name),datasets:[
      {label:'Profit ($)',data:trend.map(d=>d.profit),borderColor:'#233B61',backgroundColor:'#233B61',yAxisID:'y',tension:0},
      {label:'Margin (%)',data:trend.map(d=>d.margin_pct),borderColor:'#2E6FD9',backgroundColor:'#2E6FD9',borderDash:[5,4],yAxisID:'margin',tension:0,spanGaps:false}
    ]},options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{position:'bottom'},tooltip:{callbacks:{label:c=>c.dataset.label+': '+(c.dataset.yAxisID==='margin'?pct(c.parsed.y):invoiceMoney(c.parsed.y))}}},scales:{y:{beginAtZero:true,title:{display:true,text:'Profit ($)'},ticks:{callback:v=>money(v)}},margin:{position:'right',title:{display:true,text:'Margin (%)'},grid:{drawOnChartArea:false},ticks:{callback:v=>v+'%'}}}}});
    const items=BranchModel.topItems(rows,state.data.invoice_drilldown),top=items?.items||[];
    $('branchItemsChart').parentElement.hidden=!top.length;
    $('branchItemsNote').textContent=items?`Invoice source lines only; returns excluded. All source line types/components retained; grouped by item ID (blank IDs grouped). ${number(items.item_count)} items. Header less all lines: ${invoiceMoney(items.difference)}; not allocated. ${number(items.missing_documents)} invoices with no source lines. Item totals may differ from header gross sales.`:'Item detail unavailable in this snapshot; header totals remain unchanged.';
    chart('branchItemsChart',{type:'bar',data:{labels:top.map(d=>d.item||'No item ID'),datasets:[{label:'Gross invoice line sales',data:top.map(d=>d.gross_sales),backgroundColor:'#2E6FD9',borderRadius:3}]},options:{indexAxis:'y',responsive:true,maintainAspectRatio:false,plugins:{legend:{display:false},tooltip:{callbacks:{afterTitle:c=>top[c[0].dataIndex].description,label:c=>invoiceMoney(c.parsed.x)}}},scales:{x:{beginAtZero:true,ticks:{callback:v=>money(v)}},y:{ticks:{autoSkip:false,callback:function(v){const label=this.getLabelForValue(v);return label.length>24?label.slice(0,23)+'…':label;}}}}}});
    $('branchTopItems').innerHTML=table(['Item / description','Gross sales'],top.map(d=>`<tr><td><b>${esc(d.item||'No item ID')}</b><div class="panel-note">${esc(d.description)}</div></td><td class="num">${amount(d.gross_sales)}</td></tr>`).join('')||'<tr><td colspan="2">No invoice item lines available for this branch and period.</td></tr>');
